...
async function startQuiz(): Promise<void> {
  ...
  for (const quiz of appData) {
    ...
    if (answer.answer === quiz.correctAnswer) {
      ...
      incorrectAnswers.push(
        `${quiz.question} ${chalk.red(
          incorrectAnswers
        )} Correct answer is ${chalk.green(quiz.correctAnswer)}`
      );
    }
  }
  ...
  if (incorrectAnswers.length > 0) {
    ...
    console.log(
      chalk.green.bold(
        "congratulations you got the all correct answers your score is",
        score
      )
    );
  }
